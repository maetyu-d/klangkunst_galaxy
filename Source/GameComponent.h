...
private:
    ...
    std::vector<bool> discoveredGalaxySystems;
    float galaxyFuel = 100.0f;
    float galaxyFuelCapacity = 100.0f;
    juce::String galaxyStatusMessage;
    int galaxyStatusMessageFrames = 0;
    ...
    int isometricCameraAnchorHeight = 2;
    bool shouldRecenterIsometricOnEntry = true;
    ...
    juce::String getPlanetTraitName (PlanetTrait trait) const;
    juce::Colour getPlanetTraitColour (PlanetTrait trait) const;
    juce::String getPlanetTraitFlavour (PlanetTrait trait) const;
    ...
    void ensureGalaxyDiscoveryState();
    void revealGalaxyAroundSelection (float radius = 0.11f);
    bool isGalaxySystemDiscovered (int systemIndex) const;
    float getFuelCostForSystemJump (int fromSystemIndex, int toSystemIndex) const;
    void setGalaxyStatusMessage (juce::String message, int frames = 180);
    void refuelFromPlanet (const PlanetMetadata& planet);
    std::vector<std::pair<int, int>> getGalaxyWarpLinks() const;
    int getWarpDestinationForSystem (int systemIndex) const;
    juce::String getGalaxyRegionName (const StarSystemMetadata& system) const;
    juce::Colour getGalaxyRegionColour (const StarSystemMetadata& system) const;
    void applyAssignedBuildModeForPlanet();
    void resetIsometricCameraForCurrentPlanet();
